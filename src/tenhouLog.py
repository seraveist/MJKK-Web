...
class log(object):
    ...
    def __init__(self, logObj, playerSum):
        ...
        if not self.isDraw:
            for lst in self.yakus:
                ...
                temp3 = 0
                for yaku in lst:
                    if yaku.startswith(("Dora", "ドラ")):         #dora_outer
                        temp1 += int(re.findall(r"\d+", yaku)[0])
                    elif yaku.startswith(("Ura Dora", "裏ドラ")): #dora_inner
                        temp2 += int(re.findall(r"\d+", yaku)[0])
                    elif yaku.startswith(("Red Five", "赤ドラ")): #dora_akai
                        temp1 += int(re.findall(r"\d+", yaku)[0])
    ...
    def isHost(self, playerIndex):
        return (self.gameWindIndex) % 4 == playerIndex
    ...
    def richiMachi(self, playerIndex):
        playerHand = generate_hand(self.logObj[4 + playerIndex * 3],self.logObj[5 + playerIndex * 3], self.logObj[6 + playerIndex * 3])
        result = len(GotHoorahai(playerHand)[0][-1])
        return result
    
    def richiFirst(self, targetIndex):
      result = (5, 99)
      for playerIndex in range(0, 4):
        for index, cID in enumerate(self.logObj[6 + playerIndex * 3]):
            if type(cID) == str:
                if "r" in cID:
                  if index < result[1]:
                    result = (playerIndex, index)

      if result[0] == targetIndex:
        return True
      else:
        return False 
        ...
        for i in range(len(self.changeScore)):
            if self.changeScore[i][playerIndex] > 0:
                for yaku in self.yakus[i]:
                    if "Ippatsu" in yaku or "一発" in yaku:
                        return True
        return False

    def doraInnerScore(self, playerIndex, dora_inner):
        score = self.logObj[16][2][3]
        fan = self.fan[0]
        fu = 40
        isHost = self.isHost(playerIndex)
        isTsumo = False
